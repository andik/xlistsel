...
int nkapp_run(nkapp_callback cb, void* userdata, int winwidth, int winheight, const char* wintitle)
{
    ...
    xw.win = XCreateWindow(xw.dpy, xw.root, 0, 0, winwidth, winheight, 0,
        XDefaultDepth(xw.dpy, xw.screen), InputOutput,
        xw.vis, CWEventMask | CWColormap, &xw.swa);

    XStoreName(xw.dpy, xw.win, wintitle);
    ...
    while (running)
    {
        ...
        while (XPending(xw.dpy)) {
            ...
            if (evt.type == ClientMessage) goto cleanup;
						...
						{
							/* Update Window Size: this is only necessary upon Resize */
							XGetWindowAttributes(xw.dpy, xw.win, &xw.attr);
							xw.width = (unsigned int)xw.attr.width;
							xw.height = (unsigned int)xw.attr.height;
						}
        ...
        nk_input_end(ctx);
				...
				/* run client app */
        if(cb(ctx, xw.width, xw.height, userdata) == 0) break;
